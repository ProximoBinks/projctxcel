...
export default function TutorResetPasswordPage() {
  return (
    <Suspense>
      <TutorResetPasswordContent />
    </Suspense>
  );
}

function TutorResetPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/tutor-reset-password", {
        method: "POST",
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 px-4 py-3 pr-11 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    required
                    minLength={8}
                    disabled={!token}
                    autoFocus
